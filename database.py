...
async def init_db():
    ...
    # Миграция для баз, созданных до появления колонки language
    try:
        await _db.execute("ALTER TABLE guild_settings ADD COLUMN language TEXT DEFAULT 'ru'")
    except Exception:
        pass  # колонка уже существует
    try:
        await _db.execute("ALTER TABLE guild_settings ADD COLUMN warn_limit INTEGER DEFAULT 0")
    except Exception:
        pass  # колонка уже существует
    await _db.commit()
