...
def update_settings(guild_id):
    ...
    mute_role_id = request.form.get("mute_role_id") or None
    ...
    db.update_guild_settings(
        ...
        mute_role_id=int(mute_role_id) if mute_role_id else None,
...
@app.route("/guild/<int:guild_id>/mute/<int:user_id>/delete", methods=["POST"])
@login_required
def delete_mute(guild_id, user_id):
    db.delete_mute(guild_id, user_id)
    flash("✅ Мьют снят / Mute removed")
    return redirect(url_for("guild_detail", guild_id=guild_id))
