...
async def autorole_setup(interaction: discord.Interaction):
    embed = discord.Embed(
        ...
        description=(
            ...
            "Как только будут заданы и канал, и роль — авто-выдача включится **автоматически**, "
            "без отдельной команды. Отключить/включить вручную можно через "
            "`/autorole disable` и `/autorole enable`."
        ),
...
async def autorole_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await _maybe_autoenable(interaction.guild_id)
...
async def _maybe_autoenable(guild_id: int):
    """Включает авто-роль автоматически, как только заданы и канал, и роль."""
    cfg = await db.get_autorole_config(guild_id)
    if cfg:
        channel_id, role_id, _custom_message, enabled = cfg
        if channel_id and role_id and not enabled:
